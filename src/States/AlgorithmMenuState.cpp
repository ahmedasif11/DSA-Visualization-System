...
AlgorithmMenuState::AlgorithmMenuState(StateContext& context)
    : State(context)
    , m_background()
...
void AlgorithmMenuState::onEnter() {
    ...
    std::cout << "AlgorithmMenuState entered" << std::endl;
}

void AlgorithmMenuState::setupButtons() {
    m_buttons.clear();
...
void AlgorithmMenuState::update(float deltaTime) {
...
void AlgorithmMenuState::render(sf::RenderTarget& target) {
...
void AlgorithmMenuState::onSortingAlgorithmsClicked() {
    ...
    eventBus.publish(sortMenuEvent);
}
